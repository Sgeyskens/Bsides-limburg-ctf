...
<head>
    ...
    <title>Movies - Friday the 13th</title>
        ...
        <main class="flex-1 px-4 md:px-8 lg:px-16 py-8 max-w-7xl w-full mx-auto">
            ...
            <div class="text-center mb-8">
                ...
                <p class="font-cinzel text-base md:text-lg text-[#D8C9AE] max-w-3xl mx-auto">
                    Explore the complete Friday the 13th film collection
                </p>
            </div>
            ...
            <div class="mb-4">
                <p id="products-count" class="font-cinzel text-sm text-gray-400">
                    {{ count($movies) }} product{{ count($movies) !== 1 ? 's' : '' }} found
                </p>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-6 mb-16">
                ...
                <aside id="filter-container" class="hidden md:block md:w-64 lg:w-72 flex-shrink-0">
                    @include('partials.product-filters', ['filterOptions' => $filterOptions ?? [], 'activeFilters' => $activeFilters ?? [], 'productType' => $productType ?? 'movie'])
                </aside>
                ...
                <div class="flex-1">
                    ...
                    <div id="products-grid"
                         class="grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 items-stretch {{ empty($movies) ? 'hidden' : '' }}">
                        ...
                        @foreach($movies ?? [] as $movie)
                            <div class="flex flex-col h-full bg-black/80 backdrop-blur-sm border-2 border-red-900/50 rounded-lg overflow-hidden transition-all hover:border-red-700 hover:scale-105 hover:shadow-xl hover:shadow-red-900/30">
                                ...
                                <div class="aspect-[2/3] bg-gradient-to-br from-red-950/30 to-black flex items-center justify-center border-b-2 border-red-900/50">
                                    @if($movie->image_url)
                                        <img src="{{ str_starts_with($movie->image_url, 'http') ? $movie->image_url : asset($movie->image_url) }}" alt="{{ $movie->name }}" class="w-full h-full object-cover">
                                    @else
                                        <img src="{{ asset('images/mask-logo.png') }}" alt="Movie Poster" class="w-20 h-20 md:w-24 md:h-24 opacity-60">
                                    @endif
                                </div>
                                ...
                                <div class="flex flex-col flex-grow p-3">
                                    ...
                                    @if(($movie->rating_count ?? 0) > 0)
                                        <div class="flex items-center gap-1 mb-2">
                                            @for($i = 1; $i <= 5; $i++)
                                                <svg class="w-3 h-3 {{ $i <= round($movie->avg_rating ?? 0) ? 'text-red-700' : 'text-gray-600' }}" fill="currentColor" viewBox="0 0 20 20">
                                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                </svg>
                                            @endfor
                                            <span class="text-xs text-gray-400">({{ $movie->rating_count }})</span>
                                        </div>
                                    @endif
                                    ...
                                    <div class="flex flex-col gap-2 mt-auto product-card">
                                        ...
                                        @auth
                                            <button data-add-to-cart data-product-id="{{ $movie->product_id }}" data-product-type="movie" class="flex items-center justify-center gap-2 px-3 py-1.5 bg-red-900/50 hover:bg-red-900 text-white font-cinzel text-xs uppercase rounded border border-red-700 transition-all hover:scale-105">
                    ...
                    <div id="no-results"
                         class="{{ empty($movies) ? '' : 'hidden' }} text-center py-20">
                        <div class="bg-gradient-to-br from-red-950/30 via-black/80 to-red-950/30 border-2 border-red-900/50 rounded-lg p-12 max-w-2xl mx-auto backdrop-blur-sm">
                            ...
                            <p class="text-gray-400 font-cinzel">
                                {{ empty($movies)
                                    ? 'Check back soon for the complete Friday the 13th collection'
