...
<head>
    ...
    <title>Create Discount Code - Admin</title>
...
<body class="bg-black text-white min-h-screen">
    ...
    <main class="max-w-3xl mx-auto px-4 md:px-8 py-8">
        <!-- Page Header -->
        <div class="mb-8">
            <h2 class="font-cinzel text-3xl md:text-4xl text-[#D8C9AE] uppercase tracking-wider font-bold">Create Discount Code</h2>
            <p class="font-cinzel text-sm text-gray-400 mt-1">Add a new discount code for customers to use</p>
        </div>
        ...
        <!-- Form -->
        <form action="{{ route('admin.discounts.store') }}" method="POST" class="bg-black/80 border-2 border-red-900/50 rounded-lg p-6 md:p-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input
                        ...
                        name="valid_until"
                        id="valid_until"
                        value="{{ old('valid_until', date('Y-m-d', strtotime('+1 month'))) }}"
                        required
                        class="w-full bg-black/50 border-2 border-red-900/50 rounded px-4 py-3 font-cinzel text-white focus:border-red-700 focus:outline-none transition-colors"
                    >
                </div>
            ...
            <div class="mb-8">
                ...
                >
                    ...
                    <option value="movie" {{ old('applies_to') === 'movie' ? 'selected' : '' }}>Movies Only</option>
                    <option value="game" {{ old('applies_to') === 'game' ? 'selected' : '' }}>Games Only</option>
                    <option value="merch" {{ old('applies_to') === 'merch' ? 'selected' : '' }}>Merch Only</option>
                </select>
            </div>
